...
The AQP control-plane refactor (see [`agentic_quant_platform/docs/architecture/decisions/005-separated-control-plane.md`](../../../agentic_quant_platform/docs/architecture/decisions/005-separated-control-plane.md)) stands up a standalone `aqp_control_plane/` micro-project that:
...
| `POST /api/cluster/.../exec`           | `POST /manage/deployments/{id}/exec`       | Pending    |
| `GET  /api/cluster/.../logs/stream`    | `WS   /manage/telemetry/stream/{id}`       | Pending    |
...
| `GET  /api/observability/*`            | `GET  /manage/telemetry/snapshot`          | Pending    |
...
4. **For operator UI integration**, the canonical Python client is now `rpi_k8s_sdk.aqp` — it points at `aqp_control_plane` HTTP API by default.
